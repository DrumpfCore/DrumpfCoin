...
int nAnonymizeDrumpfCoinAmount = 1000;
...
#else
    const char* pszModule = "drumpfcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DrumpfCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DrumpfCoin
    // Mac: ~/Library/Application Support/DrumpfCoin
    // Unix: ~/.drumpfcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DrumpfCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "drumpfcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "drumpfcoind.pid"));
